...
int pedir_asientos(int asiento_habilitado[MAX_ASIENTOS], int asientosEnSala,
                   int asientos_elegidos[MAX_ASIENTOS_RESERVADOS]);

int pedir_sala(int asientos_por_sala[MAX_SALAS], int cant_salas);

void mostrar_asientos_reservados(int asientos_elegidos[MAX_ASIENTOS_RESERVADOS], int cantAsientos,
...
int main() {
    ...
    result = m_asientos_sala(m_id, nro_sala, asiento_habilitado, &asientosEnSala);
    if (result != 0) {
        CLI_PRINTF("ERROR: no se pudo obtener la info de las sala elegida");
    }
    mostrar_asientos(asiento_habilitado);
    /* ELEGIR ASIENTOS DENTRO DE LA SALA ELEGIDA */

    int asientos_elegidos[MAX_ASIENTOS_RESERVADOS];
    int cant_asientos = pedir_asientos(asiento_habilitado, asientosEnSala, asientos_elegidos);
    int asientos_reservados[MAX_ASIENTOS_RESERVADOS];
    int cant_reservados;
    result = m_reservar_asientos(m_id, asientos_elegidos, cant_asientos, nro_sala, asientos_reservados,
                                 &cant_reservados);
    if (result != 0) {
        CLI_PRINTF("ERROR: no se pudo reservar los asientos");
    }
    printf("\nSe reservaron %i/%i asientos: ", cant_reservados, cant_asientos);
    mostrar_asientos_reservados(asientos_elegidos, cant_asientos, asientos_reservados, cant_reservados);
...
void mostrar_asientos_reservados(int asientos_elegidos[MAX_ASIENTOS_RESERVADOS], int cantAsientos,
                                 int asientos_reservados[MAX_ASIENTOS_RESERVADOS], int cant_reservados) {
    printf("\nSe reservaron %i/%i asientos: ", cant_reservados, cantAsientos);
