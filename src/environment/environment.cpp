...
int main(int argc, char *argv[]) {
	if (argc != 3) {
		std::cout << "Usage: " << argv[0] << " --[start|stop] [cliente|cine]" << std::endl;
		return -1;
	}
	...
	if (std::strcmp(argv[1], "--start") == 0) {
		...
		if (std::strcmp(argv[2], "cliente") == 0) {
			...
			if (!success) {
				...
				std::cout << "Se recomienda ejecutar --stop cliente y probar otra vez" << std::endl;
				return -2;
			}

			std::cout << "Se crearon todos los IPC para el cliente" << std::endl;
			return 0;
		}
		else if (std::strcmp(argv[2], "cine") == 0) {
			/* Intento crear todas. Si alguna falla, sigo intentando con las demas */
			...
			if (!success) {
				std::cout << "Se encontraron errores al crear algunos IPC (posiblemente ya existian)\n";
				std::cout << "Se recomienda ejecutar --stop cine y probar otra vez" << std::endl;
				return -2;
			}

			std::cout << "Se crearon todos los IPC para el cine" << std::endl;
			return 0;
		}
	}
